...
class FUNITModel(nn.Module):
    ...
    def forward(self, co_data, cl_data, hp, mode="dis_update"):
        xa = co_data[0].cuda()
        la = co_data[1].cuda()
        xb = cl_data[0].cuda()
        lb = cl_data[1].cuda()
        if mode == 'gen_update':
            ...
            s_xa = self.gen.enc_class_model(xa, xa)
            s_xb = self.gen.enc_class_model(xa, xb)
            ...
            xb.requires_grad_()
            l_real_pre, acc_r, resp_r = self.dis.calc_dis_real_loss(xb, lb)  # loss, acc, [B, 4, 4]
            l_real = hp['gan_w'] * l_real_pre   # 1 * loss
            l_real.backward(retain_graph=True)  # backward
            l_reg_pre = self.dis.calc_grad2(resp_r, xb)
            l_reg = 10 * l_reg_pre
            l_reg.backward()                    # WGAN-GP loss
            ...
            l_fake.backward()
            l_total = l_fake + l_real + l_reg       # real and fake data loss + WGAN_PG
            acc = 0.5 * (acc_f + acc_r)             # dis의 real and fake data accuracy
    ...
    def test(self, co_data, cl_data):
        ...
        s_xa_current = self.gen.enc_class_model(xa, xa)
        s_xb_current = self.gen.enc_class_model(xa, xb)
        ...
        s_xa = self.gen_test.enc_class_model(xa, xa)
        s_xb = self.gen_test.enc_class_model(xa, xb)
    ...
    def translate_k_shot(self, co_data, cl_data, k):
        ...
        if k == 1:
            c_xa_current = self.gen_test.enc_content(xa)
            s_xb_current = self.gen_test.enc_class_model(xa, xb)
            xt_current = self.gen_test.decode(c_xa_current, s_xb_current)
        else:
            s_xb_current_before = self.gen_test.enc_class_model(xa, xb)
