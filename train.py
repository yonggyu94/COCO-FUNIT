...
output_directory = os.path.join(opts.output_path + "/output", model_name)
        ...
        if ((iterations + 1) % config['image_save_iter'] == 0 or (
            ...
            with torch.no_grad():
                ...
                for t, (test_co_data, test_cl_data) in enumerate(
                    ...
                    img_grid_co = torchvision.utils.make_grid(co_data[0])
                    img_grid_cl = torchvision.utils.make_grid(cl_data[0])

                    train_writer.add_image('co_data', img_grid_co)
                    train_writer.add_image('cl_data', img_grid_cl)
